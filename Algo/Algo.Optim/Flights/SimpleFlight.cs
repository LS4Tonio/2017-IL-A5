...
namespace Algo.Optim
{
    public class SimpleFlight
    {
        ...
        public static IList<SimpleFlight> Load(string path)
        {
        ...
        public static void Save(IEnumerable<SimpleFlight> flights, string path)
        {
            var settings = new XmlWriterSettings()
            {
                Indent = true
            };
            using (var r = XmlWriter.Create(path, settings))
            {
                ...
                foreach (var f in flights)
                {
                    ...
                    r.WriteElementString("price", f.Price.ToString(CultureInfo.InvariantCulture));
